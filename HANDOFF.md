...
> [How to update](#how-to-update-this-file) at the bottom). Last updated: **2026-07-09** (round 32).
...
| **Learn (play-along)** — Yousician-class trainer with our OWN animation: a **strum highway** (chord + ↓/↑ arrows flow to a strike line in tempo, down=copper/up=green, pulse on cross) + count-in. 5th **Learn** nav tab (built-in lessons + today's challenge as a playable lesson). Pure `LessonTiming` + `Ticker`-driven player. **Scoring is next (round 33).** (chunk 014) | ✅ **round 32** | `lib/features/learn/` |
| **Tests** | ✅ **179 Flutter + 14 backend green** (widget + DSP unit + chord-dictionary + Viterbi + extended-chord + randomized property (9-seed verified) + auth/sync + analyze/library + capo/transpose + share-card + streak/challenge + onboarding + learn/play-along + pytest) | `test/`, `backend/tests/` |
