...
//     next onset's timestamp and the truncation is SIGNALLED so the
//     downstream code can decide whether to use the truncated window
//     (most cases) or discard it (rare alignment work).
//
...
final class StrokeWindow {
  ...
  /// half-open interval `[onset, nextOnset)` so the next onset's pre-window
  /// cannot leak into the current window.
  ///
  ...
  List<PickingStrokeCut> cut({
    ...
    for (var i = 0; i < sortedOnsets.length; i++) {
      ...
      if (truncatedToNext != null &&
          truncatedToNext.inMicroseconds < requestedEnd.inMicroseconds) {
        actualEnd = truncatedToNext;
        truncated = true;
      } else {
