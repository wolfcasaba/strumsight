...
class YinPitchDetector {
  ...
  /// Clarity of the last [detect] call, or zero when no pitch is found.
  double clarity = 0;

  /// Estimates f0 in Hz, or returns null when no threshold dip is present.
  double? detect(Float64List buffer) {
    assert(buffer.length >= bufferSize);
    final w = bufferSize - _maxLag;
...
/// Maps a frequency to its nearest pitch-class note and cents offset.
