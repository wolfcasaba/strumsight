...
## 0.0 Pre-flight brief-revízió (orchestrátor, 2026-08-19) — öt mért pont
...
| R5 | a 7. Gate 2. sora (`python3 -m pytest tools/tests -q`) és a DoD #3 szó szerint az IMPLEMENTERnek is elő tudná írni ugyanezt a parancsot önellenőrzésre | **mérve, ezen a boxon**: a rendszer `python3`-ban NINCS `pytest` modul (`ModuleNotFoundError`), és a `pip install` — akár egy eldobható venv-be is — az implementer-preambulum §4-e SZERINT tilos, és a gépi `tools/hooks/implementer_guard.py` `FORBIDDEN_BASH` mintája (`\bpip3?\s+install\b`) EGY venv-lokális `.../bin/pip install`-ra is illeszkedik, tehát nem csak szövegesen, hanem a hook szintjén is le van tiltva ebben a körben — nincs kikerülő út | **az implementer a SAJÁT, ÚJ tesztfájljait `python3 -m unittest tools.tests.test_round_metrics_granularity` / `python3 -m unittest tools.tests.test_brief_merge_plan` alakkal ellenőrizze `done` előtt** (stdlib, verifikálva ezen a boxon egy meglévő fájlon: `python3 -m unittest tools.tests.test_round_metrics_engines -v` → 7/7 OK, csomagtelepítés nélkül) — ez NEM helyettesíti a brief 7. Gate sorát, csak az implementer csomagtelepítés nélküli önellenőrzését adja. A szó szerinti `python3 -m pytest tools/tests -q` parancsot (izolált, eldobható venv-ben) az orchestrátor/review futtatja a kör UTÁN — ez NEM implementer-feladat, és a hiányzó `pytest`-modul emiatt NEM `stopped`-ot indokol |
