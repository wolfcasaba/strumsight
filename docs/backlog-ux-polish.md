...
| 4 | **Analyze/Progress loading skeleton** (shimmer timeline while analyzing, not just a spinner) | `analyze/screens/analyze_screen.dart` | M | ✅ r189 |
| 5 | **Hardcoded hero colors bypass theme tokens** (breaks the existing light theme) — add `onPrimary`/`inkOnAccent` to `AppPalette` | `live_screen.dart:255`, analyze, lesson_highway, streak_badge, strum_card | S-M | ✅ r189 |
...
| 13 | **Metronome shortcut leaves Live listening** — `onMetronome` still `context.push`es raw while the Tuner shortcut goes through `_openCovered`, so Live keeps the microphone open and the wakelock held while it is covered. No lease conflict (the Metronome takes no mic) and no broken feature — the same battery/privacy shape as the Tuner defect, wanting the same pause/handover/resume | `live/screens/live_screen.dart` `onMetronome` → `_openCovered` | S | ✅ 2026-09-18 (hibavadászat) |
