...
| 3/b | `release_flow_text_scale_test` en+hu @2.0: `practice_setup_screen.dart:424` 43 px | az E12-R20 ismert kivétel SORSZÁMRA volt tűzve (`:418`); az F4 `_backToHub`-javítása 6 sort tolt → a tolerancia elavult → a cella (helyesen) piros | **a hibát javítottam, nem a tűt mozgattam**: `_ScoringProfileReadout` azonosítója a sor 60 %-ára korlátozva tördelődik (`LayoutBuilder` + `ConstrainedBox`; természetes szélességnél a pixel-golden változatlan — az első, `Flexible`-alapú próbálkozás az `e13_r21_practice_setup_compact` goldent 0,9 %-kal, a második `textAlign: end`-del 240 px-nyi szubpixel-eltolással mozdította el, ezért lett mindkettő cserélve); a bejegyzés kikerült a `knownOverflows` tükörből és a `known-exceptions.yaml`-ból (zsugorodó registry, A6 őr); `release-audit.md` + `known-issues.md` frissítve |
| 4 | `e13_r21_practice_setup_compact` golden 0,9 % → 0,06 % → zöld | a 3/b első két megoldása pixelt mozdított (goldent csak a user boxa regenerál) | `LayoutBuilder` + `ConstrainedBox(maxWidth: 60 %)`, `textAlign` nélkül — a nem-nagyított render bájtra azonos |
| záró | `3521c210`: [build-apk 34431041159](https://github.com/wolfcasaba/strumsight/actions/runs/34431041159) **ZÖLD** (10317 teszt, 0 piros, 21 skip; property-kapu 123 zöld random seeddel; APK-artefakt 10134964374); [full-gate 34431039782](https://github.com/wolfcasaba/strumsight/actions/runs/34431039782): `full-gate` job **ZÖLD**, a Coverage job 1 rejtett, a másik három futásban nem reprodukálódó cellával piros | nemdeterminisztikus cella a `--coverage` futásban (azonos SHA, azonos suite, 3/4 zöld) | egyszeri újrafuttatás (attempt 2) **ZÖLD** → a teljes `full-gate` workflow zöld; az azonosításhoz a teljes log kellene (a GitHub log-API 5000 soros plafonja miatt a szelet-módszer sem éri el, mert szeletben nem reprodukálódott) |
