...
| `lib/features/community/presentation/screens/clubs/club_detail_screen.dart` | `ClubDetailScreen` | no | no | unreachable | — | — | Reachable only from a club list item, which is not yet wired; no measured construction site in lib/. |
...
| `lib/features/community/presentation/screens/clubs/club_member_management_screen.dart` | `ClubMemberManagementScreen` | no | no | unreachable | — | — | Reachable only from club detail, which is itself unrouted; no measured construction site in lib/. |
...
| `lib/features/community/presentation/screens/community_challenges_screen.dart` | `CommunityChallengesScreen` | no | no | unreachable | — | — | NOT routed, and that is a measurement: the hosted instance serves no challenge list or detail, so a route would be a button that always fails. It was previously reported reachable because the scan matched its class name inside the comment saying so (E18-R22). |
...
| `lib/features/community/presentation/screens/community_search_screen.dart` | `CommunitySearchScreen` | no | no | unreachable | — | — | No measured construction site in lib/: search has no entry point on any routed community screen yet. |
| `lib/features/community/presentation/screens/edit_profile_screen.dart` | `EditProfileScreen` | no | no | unreachable | — | — | No measured construction site in lib/: the profile surface that would open it is not routed. |
...
| `lib/features/community/presentation/screens/post_composer_screen.dart` | `PostComposerScreen` | no | no | unreachable | — | — | No measured construction site in lib/: composing needs a write entry point that no routed screen offers yet. |
