...
`f154139`). Huszonhárom kör futott le rajta (az R9 három részletben: /1, /2, /3;
...
- **Az AI Tanár felhő-kapuja — a KLIENS fele és az üzemeltetői flip**
  (újra-audit M2). **A backend-fél az R23-mal ZÁRVA:** a
  `main.py::_build_tutor_gateway` a konfigurált adaptert állítja fel, a valódi
  `AnthropicProviderGateway` létezik (osztályozott hibaleképezés, kulcs- és
  ...
  `FakeProviderGateway` épülne" állítás tehát MÁR NEM igaz. Nyitva maradt KETTŐ:
  **(a) üzemeltetői művelet** — az alapértelmezés `tutor_provider=fake` és
  ...
  `STRUMSIGHT_TUTOR_ENABLED=true`) — a lépéssor runbook **§7.2**, és a flip ELŐTT
  a `docs/privacy/data-inventory.yaml` `tutor_stream` sorának fel kell sorolnia a
  harmadik fél feldolgozóját + a megőrzést (a runbook ezt BLOKKOLÓKÉNT jelöli);
  **(b) a kliens fele** — a `selectTutorModelGateway` továbbra sem olvassa az
  `aiTutorCloudEnabled`-et (a zászlónak mérve nulla fogyasztója van a
  `feature_flags.dart`-on kívül), tehát a felhő-kapu a kliensen nincs bekötve.
