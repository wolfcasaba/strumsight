...
| `lib/features/ai_tutor/presentation/screens/practice_plan_preview_screen.dart` | R9 (2026-09-07) óta van `PracticePlanDraft`-előállítója (`TutorPracticePlanProducer`, a beépített gyakorlat-katalógusból) és útvonala (`/tutor/plan-preview`, a `PracticePlanPreviewRoute` kompozíción át) — eddig sem draftot, sem route-ot, sem konstrukciós helyet nem kapott. `aiTutorEnabled` KI besorolású (docs/release/capability-rollout.md), a bekapcsolás külön Termék/User döntés (Epic 4 zárójelentés), és a képernyőn lévő belépési pont hiányzik: minden szóba jövő AI Tutor képernyő pixel-rögzített a golden variáns-mátrixban, ezért a CTA külön, tulajdonos-kör dolga. A bejárás mag-útja (§1) Today/Practice/Library/Progress/Profile állomásokat nevez meg; a bekötést a `test/features/ai_tutor/presentation/practice_plan_preview_route_test.dart` méri. | AI Tutor feature (Epic 4) | nincs — GA flag-flip Termék/User döntés (docs/sdd/epic-04-completion-report.md) |
