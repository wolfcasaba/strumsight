...
| `lib/features/song_trainer/presentation/screens/setlist_list_screen_v2.dart` | R10 (2026-09-07) óta van útvonala (`/setlists/v2`, illetve `/setlists/v2/session`): a Setlist V2 lista és az általa indított rendezett session eddig sem route-tal, sem konstrukciós hellyel nem rendelkezett. A bejárás mag-útja (§1) Today/Practice/Library/Progress/Profile állomásokat nevez meg, a Setlist V2 nincs köztük; a bekötést a `test/features/song_trainer/setlist/setlist_session_launch_test.dart` méri. | Song Trainer feature (Setlist V2) | nincs — a Setlist V2 a mag-úton kívül esik; a képernyőn lévő belépési pont R12 (2026-09-07) óta megvan — a legacy `songs/setlist_list_screen.dart` görgethető törzsének `setlist-open-v2` kártyája, mérve a `test/features/songs/setlist_v2_entry_test.dart` V1–V3 cellájával |
| `lib/features/song_trainer/presentation/screens/setlist_session_screen.dart` | R10 (2026-09-07) óta van útvonala (`/setlists/v2`, illetve `/setlists/v2/session`): a Setlist V2 lista és az általa indított rendezett session eddig sem route-tal, sem konstrukciós hellyel nem rendelkezett. A bejárás mag-útja (§1) Today/Practice/Library/Progress/Profile állomásokat nevez meg, a Setlist V2 nincs köztük; a bekötést a `test/features/song_trainer/setlist/setlist_session_launch_test.dart` méri. | Song Trainer feature (Setlist V2) | nincs — a Setlist V2 a mag-úton kívül esik; a képernyőn lévő belépési pont R12 (2026-09-07) óta megvan — a legacy `songs/setlist_list_screen.dart` görgethető törzsének `setlist-open-v2` kártyája, mérve a `test/features/songs/setlist_v2_entry_test.dart` V1–V3 cellájával |
...
| `lib/features/ai_tutor/presentation/screens/practice_plan_preview_screen.dart` | R9 (2026-09-07) óta van `PracticePlanDraft`-előállítója (`TutorPracticePlanProducer`, a beépített gyakorlat-katalógusból) és útvonala (`/tutor/plan-preview`, a `PracticePlanPreviewRoute` kompozíción át) — eddig sem draftot, sem route-ot, sem konstrukciós helyet nem kapott. `aiTutorEnabled` KI besorolású (docs/release/capability-rollout.md), a bekapcsolás külön Termék/User döntés (Epic 4 zárójelentés), a képernyőn lévő belépési pont R12 (2026-09-07) óta megvan: a `tutor_profile_screen.dart` `tutorProfilePlanPreview` kártyája, ugyanazon `aiTutorEnabled` kapu alatt, amivel a route regisztrálva van (a TutorHome és a TutorChat pixel-rögzített, ezért nem ott). A bejárás mag-útja (§1) Today/Practice/Library/Progress/Profile állomásokat nevez meg; a bekötést a `test/features/ai_tutor/presentation/practice_plan_preview_route_test.dart` méri. | AI Tutor feature (Epic 4) | nincs — GA flag-flip Termék/User döntés (docs/sdd/epic-04-completion-report.md) |
