...
- **`@Deprecated` sites and external-callsite counts** are measured by
  ...
  A file with **zero** external callsites has no in-tree consumer left; it
  is not, by itself, permission to delete it (§5.3 below).
- **Parallel-layer (`*_v2`) callsite counts** are measured the same way,
  at directory granularity (any file importing something under
...
- All counts below are as of **2026-09-02** (`main @ 496264d9`) — re-run
  `dart run tool/check_deprecations.dart` before acting on any row; a
  callsite count can only have gone up or down since.
...
|---|---|---|---|---|
| `wav.dart` re-export shim (`@Deprecated`, points at `core/audio/codec/wav_codec.dart`) | `lib/features/learn/audio/wav.dart` | `lib/core/audio` (codec ownership) | `dart run tool/check_deprecations.dart` reports 0 external callsites for this file AND a dedicated cleanup round deletes it (this round's §4 forbids `lib/` deletion). | 0 external callsites |
| `guitar_strings.dart` re-export shim (`@Deprecated`, points at `core/music/guitar_strings.dart`) | `lib/features/tuner/model/guitar_strings.dart` | `lib/core/music` | Same condition as the row above, applied to this file. | 0 external callsites |
| `tuning.dart` re-export shim (`@Deprecated`, points at `core/music/tuning.dart`) | `lib/features/tuner/model/tuning.dart` | `lib/core/music` | Same condition as the `wav.dart` row, applied to this file. | 0 external callsites |
| `wav_decoder.dart` re-export shim (`@Deprecated`, points at `core/audio/codec/wav_codec.dart`) | `lib/features/analyze/engine/wav_decoder.dart` | `lib/core/audio` (codec ownership) | Same condition as the `wav.dart` row, applied to this file. | 0 external callsites |
| `sliding_framer.dart` re-export shim (`@Deprecated`, points at `core/audio/dsp/sliding_framer.dart`) | `lib/features/live/engine/dsp/sliding_framer.dart` | `lib/core/audio` (DSP ownership) | Same condition as the `wav.dart` row, applied to this file. | 0 external callsites |
| `chord.dart` re-export shim (`@Deprecated`, points at `core/music/chord.dart`) | `lib/features/live/model/chord.dart` | `lib/core/music` | Same condition as the `wav.dart` row, applied to this file. | 0 external callsites |
| `chord_event.dart` re-export shim (`@Deprecated`, points at `core/music/chord_event.dart`) | `lib/features/live/model/chord_event.dart` | `lib/core/music` | Same condition as the `wav.dart` row, applied to this file. | 0 external callsites |
| `strum.dart` re-export shim (`@Deprecated`, points at `core/music/strum.dart`) | `lib/features/live/model/strum.dart` | `lib/core/music` | Same condition as the `wav.dart` row, applied to this file. | 0 external callsites |
| `ApiConfig` legacy compatibility class (4 `@Deprecated` members, points at `AppConfig`/`appConfigProvider`) | `lib/core/api/api_config.dart` | `lib/app/config` (`AppConfig` ownership) | `dart run tool/check_deprecations.dart` reports 0 external callsites for this file (already true, measured 2026-09-02) AND a dedicated cleanup round deletes it — the file's own doc comment already names this as the Epic 1 config-migration end state. | 0 external callsites |
| `library` vs `library_v2` parallel feature layers | `lib/features/library_v2/` | `lib/features/library_v2` | The 5 remaining `lib/features/library/` external callsites migrate to `library_v2`, `dart run tool/check_deprecations.dart` reports 0 for `lib/features/library/`, and a dedicated round removes the old layer — an ADR authorising the removal (mirroring ADR 0395's pattern for closing a compatibility layer). | `library`: 5 external callsites · `library_v2`: 1 external callsite |
| `progress` vs `progress_v2` parallel feature layers | `lib/features/progress_v2/` | `lib/features/progress` / `progress_v2` (Progress domain) | `progress_v2` currently has **zero** external callsites — this alone is NOT removal permission for either layer (§5.3): either (a) `progress_v2` gains at least one real external callsite (it is being actively wired up), or (b) a dedicated round measures and demonstrates it is genuinely dead code and removes it, or (c) once `progress_v2` is the active layer, `progress`'s 17 external callsites migrate to it and a dedicated round removes `progress`. | `progress`: 17 external callsites · `progress_v2`: 0 external callsites |
...
reproduces every callsite count in the table above from the live tree. The
nine `@Deprecated` shims already show **zero** external callsites, which
