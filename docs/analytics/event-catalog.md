...
| `recognitionQualityReported` | detection | An opt-in beta recognition-quality AGGREGATE for one window: band, model, quality bucket, bucketed accepted/rejected counts, verdict-latency bucket, user-correction flag (E14-R41, ADR 0591). Never a per-verdict trace and never audio. |
