...
def build_community_router(settings) -> APIRouter | None:
    ...
    * ``community_clubs_enabled`` — gates the whole ``clubs`` router
    ...
    # alapon maradtak all-or-nothing viszonyban a fő kapuval. A szerződést a
    # `tests/community/test_notification_router.py` méri: a `Settings`
    # alapértelmezése `community_writes_enabled=False`, és ott a
    # `POST /{public_id}/read` VÁRHATÓAN működik.
