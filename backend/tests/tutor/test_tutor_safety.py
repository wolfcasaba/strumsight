...
from app.tutor.redaction import ContentSizeGuard, RedactionRule, Redactor
...
class TestRedactor:
    ...
    def test_redacts_api_key_pattern(self):
        redactor = Redactor(
            rules=[
                RedactionRule(
                    pattern=r"sk-[a-zA-Z0-9]{8,}", replacement="[REDACTED_API_KEY]"
                ),
            ]
        )
    ...
    def test_redacts_multiple_patterns(self):
        redactor = Redactor(
            rules=[
                RedactionRule(
                    pattern=r"sk-[a-zA-Z0-9]{8,}", replacement="[REDACTED_KEY]"
                ),
                RedactionRule(
                    pattern=r"Bearer\s+[A-Za-z0-9\-._~+/]+=*",
                    replacement="[REDACTED_TOKEN]",
        ...
        result = redactor.redact("Key: sk-abcdef123456 and Bearer tok123.xyz")
    ...
    def test_redaction_report_has_redacted_flag(self):
        ...
        text, was_redacted = redactor.redact_with_report("Key sk-abc123def456 here.")
    ...
    def test_redaction_report_false_for_clean_text(self):
        ...
        text, was_redacted = redactor.redact_with_report("Clean safe text here.")
