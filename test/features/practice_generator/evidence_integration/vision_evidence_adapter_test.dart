...
void main() {
  ...
  /// the adapter's contract starts at `VisionEvidenceFact`, so this is
  /// the right level to test the adapter's rules.
  class StubReader implements VisionEvidenceReader {
    StubReader(this._facts, [this._warnings = const []]);
    final List<VisionEvidenceFact> _facts;
    final List<VisionEvidenceReaderWarning> _warnings;

    @override
    VisionEvidenceReaderOutcome read(
      List<VisionEvidenceObservationView> _,
    ) =>
  ...
  VisionEvidenceAdapter adaptedTo({
    ...
    List<VisionEvidenceReaderWarning> warnings = const [],
      ...
      VisionEvidenceAdapter(
        reader: StubReader(facts, warnings),
        measurementVersion: 1,
        allowedMetricKeys: allow ??
            const {'posture:shoulderAsymmetry', 'posture:torsoLean'},
        experimentalConfidenceCeiling: ceiling,
      );

  group('VisionEvidenceAdapter — no raw media (A1)', () {
    test(
      'serialized SkillEvidence never carries a raw-media field',
      () {
        final adapter = adaptedTo(facts: [fact()]);
        final result = adapter.adapt(<VisionEvidenceObservationView>[],
            asOf: now());
        ...
        for (final e in result.evidence) {
          ...
          for (final forbidden in const <String>[
            ...
            expect(
              ...
              isFalse,
              reason: 'A1 violation: "$forbidden" must NEVER leak into a '
  ...
  group('VisionEvidenceAdapter — graceful empty when vision is absent (A2)', () {
    test(
      ...
      () {
        ...
        expect(result.evidence, isEmpty,
            reason: 'A2: vision off must never crash, and must never '
    ...
    test('the adapter uses only narrow Vision types (A8)', () {
      ...
      expect(result.evidence.single.sourceOutcomeId.value,
          startsWith('vision:'));
    });
  });
  ...
  group('VisionEvidenceAdapter — explicit allowlist (A5)', () {
    test(
      'a fact whose metricKey is NOT in the allowlist becomes a warning and '
      'no SkillEvidence',
      () {
        ...
        expect(result.warnings
                .where((w) => w.code == 'metricNotAllowed')
                .length,
    ...
    test(
      'a metric in the allowlist becomes a SkillEvidence with the canonical '
      ...
      () {
        ...
        expect(result.evidence.single.source, EvidenceSource.vision);
        expect(result.evidence.single.performance!.metricCode,
  ...
  group('VisionEvidenceAdapter — capability missing / empty (A7)', () {
    test(
      'reader-level notObservable warnings are surfaced verbatim, zero '
      'SkillEvidence (A7)',
      () {
        final adapter = adaptedTo(
          facts: const [],
  ...
  group('VisionEvidenceAdapter — experimental ceiling', () {
    test(
      'an experimental fact cannot exceed the configured ceiling',
      () {
        final adapter = adaptedTo(
          facts: [
            fact(
        ...
        expect(result.evidence.single.confidence, 0.5,
            reason: 'experimental confidence is bounded by the ceiling; '
                'it cannot drive focus on its own');
      },
    );

    test(
      'an observed fact at high confidence is admitted at its reported '
      'confidence (uncapped)',
      () {
        final adapter = adaptedTo(
          facts: [fact(confidence: 0.9)],
          ceiling: 0.5,
        );
        final result = adapter.adapt(<VisionEvidenceObservationView>[],
            asOf: now());
