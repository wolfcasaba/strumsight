...
void main() {
  group('Gamification repository — versioned documents', () {
    test(
      ...
      () async {
        ...
        expect(store.writeLog, isNot(contains('remove:${GamificationStorageKeys.profileSnapshot}')));
        expect(
          _profileSnapshot(_repository(store)).totalXp,
          previous.totalXp,
        );
      },
    );

    test('A2: a corrupt profile snapshot is reported and read leaves its bytes intact', () {
    ...
    test('A3: every persisted gamification DTO rejects a future schema version', () {
    ...
    test('A3: all four documents round-trip their current model version', () async {
      ...
      expect(_profileSnapshot(reloaded).schemaVersion, gamificationStorageSchemaVersion);
      expect(_catalogVersion(reloaded).schemaVersion, gamificationStorageSchemaVersion);
      expect(_inbox(reloaded).value.single.schemaVersion, gamificationStorageSchemaVersion);
      expect(_migrationState(reloaded).value.schemaVersion, gamificationStorageSchemaVersion);
    });
  });
  ...
  group('Gamification repository — repository boundary', () {
    ...
    test('A6: a profile watcher publishes the fresh value after replacement', () async {
  ...
  group('Gamification repository — bounded reward inbox', () {
    test('A7: the inbox limit is inclusive and only trims the oldest over capacity', () async {
      final repository = _repository(InMemoryKeyValueStore());
    ...
    test('A8: all four gamification storage keys are unique and centralized in the schema', () {
...
GamificationProfileSnapshot _profileSnapshot(GamificationRepository repository) {
...
GamificationInboxItem _inboxItem(String id) => GamificationInboxItem(
  id: id,
...
final class _FakeGamificationRepository implements GamificationRepository {
  _FakeGamificationRepository(GamificationProfileSnapshot profile)
    : _profile = GamificationRead<GamificationProfileSnapshot>.available(profile);
  ...
  @override
  GamificationRead<GamificationProfileSnapshot> readProfileSnapshot() => _profile;
...
final class _InterruptingStore implements KeyValueStore {
  ...
  Future<void> writeString(String key, String value) {
    if (key == GamificationStorageKeys.profileSnapshot && interruptNextProfileWrite) {
  ...
  @override
  Future<void> writeInt(String key, int value) => _delegate.writeInt(key, value);
  ...
  @override
  Future<void> writeBool(String key, bool value) => _delegate.writeBool(key, value);
