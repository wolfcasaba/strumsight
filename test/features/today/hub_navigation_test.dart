...
/// A day whose plan names NO curriculum rung — the state in which the hub's
/// primary CTA delegates to `practiceStartLocation` instead of continuing a
/// named mission.
final List<Override> _noMissionPlan = <Override>[
...
void main() {
  ...
  // Audit L1 (E14) — "Start your first practice" (the first CTA a new user
  // ever sees) used to land on the Practice HUB, one more decision away from
  ...
  //
  // INTEGRATION NOTE (2026-09-19): this rule applies only where the day's
  // plan names NO curriculum rung. When it does, the hero copy names that
  // rung and the CTA continues it (`/curriculum`) — "a button labelled
  // 'continue' that goes somewhere else is the hub lying about what it just
  // offered" (`TodayPlanSnapshot.recommendedMissionId`). Every cell below
  // therefore pins a rung-less plan, which is the state this finding was
  // measured in.
  group('audit L1 — the Today CTA starts the recommended practice', () {
    testWidgets(
      ...
      (tester) async {
        final router = await _pumpTodayHub(tester, overrides: _noMissionPlan);
