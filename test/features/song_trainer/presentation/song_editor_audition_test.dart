...
final class _BackingPicker implements FilePickerAdapter {
  ...
  @override
  Future<ImportSourceFile?> pickSongFile() async => _backing();

  /// The backing-track attachment goes through the AUDIO picker (its own
  /// 32 MiB profile), so the fake answers on both entry points.
  @override
  Future<ImportSourceFile?> pickAudioFile() async => _backing();

  ImportSourceFile _backing() => ImportSourceFile(
