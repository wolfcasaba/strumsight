...
void main() {
  test('practice domain has no ambient IO, nondeterminism, or app imports', () {
    final domainDirectory = Directory('lib/features/practice/domain');
    ...
    expect(
      violations,
      isEmpty,
      reason: 'Practice domain purity violations:\n${violations.join('\n')}',
    );
  });

  test('purity scan ignores forbidden spellings in comments and strings', () {
    const source = r'''
/// Documentation may explain why print( is forbidden.
...
''';
  ...
  test('purity scan recognizes root l10n and Riverpod imports', () {
    expect(
      _findPurityViolations(
        'memory.dart',
        "import 'package:strumsight/l10n.dart';",
      ),
      ['memory.dart:1: localization import'],
    );
    expect(
      _findPurityViolations(
        'memory.dart',
      ...
      ['memory.dart:1: framework or storage import'],
    );
  });

  test('purity scan inspects executable string interpolation bodies', () {
    const source = r'''String timestamp() => '${DateTime.now()}';
String logValue() => '${print('x')}';
String literalOnly() => "${'DateTime.now()'}";''';

    expect(_findPurityViolations('memory.dart', source), [
      'memory.dart:1: ambient wall clock',
      'memory.dart:2: console output',
    ]);
  });
}
...
/// Marks source characters that are outside comments and string literals.
