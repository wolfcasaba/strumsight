...
void main() {
  ...
  group('TutorSafetyPolicy — pinned safety category cells', () {
    test('pain-response: blocks AI-as-victim text', () {
      final result = policy.evaluate(const TutorSafetyPolicyRequest(
        outputText: 'I feel pain when you play out of tune. It hurts my circuits.',
        hasInjection: false,
      ...
      expect(result.blockedCategories, contains(TutorSafetyCategory.painResponse));
      expect(result.blockedCategories, isNot(contains(TutorSafetyCategory.promptInjection)));
    });
    ...
    test('medical-refusal: blocks medical advice request', () {
      final result = policy.evaluate(const TutorSafetyPolicyRequest(
        outputText: 'I can diagnose your wrist pain — it is carpal tunnel syndrome.',
        hasInjection: false,
        hasInventedMetric: false,
        hasCameraClaim: false,
        isRateLimited: false,
      ...
      expect(result.isAllowed, isFalse);
      expect(result.blockedCategories, contains(TutorSafetyCategory.medicalRefusal));
    });
    ...
    test('copyright: blocks copyrighted content request', () {
      final result = policy.evaluate(const TutorSafetyPolicyRequest(
        outputText: 'Here are the copyrighted lyrics to Stairway to Heaven...',
        hasInjection: false,
        hasInventedMetric: false,
        hasCameraClaim: false,
        isRateLimited: false,
    ...
    test('copyright: does NOT block legitimate tab reference', () {
      final result = policy.evaluate(const TutorSafetyPolicyRequest(
        outputText: 'Try practicing the G major scale with alternate picking.',
        hasInjection: false,
        hasInventedMetric: false,
        hasCameraClaim: false,
        isRateLimited: false,
    ...
    test('credential-request: blocks password ask', () {
      ...
      expect(result.isAllowed, isFalse);
      expect(result.blockedCategories, contains(TutorSafetyCategory.credentialRequest));
    });
    ...
    test('prompt-injection: blocks when injection flag is set', () {
      ...
      expect(result.isAllowed, isFalse);
      expect(result.blockedCategories, contains(TutorSafetyCategory.promptInjection));
    });
    ...
    test('invented-metric: blocks when flag is set', () {
      ...
      expect(result.isAllowed, isFalse);
      expect(result.blockedCategories, contains(TutorSafetyCategory.inventedMetric));
    });

    test('invented-metric: hard block, never pass-with-warning', () {
      final result = policy.evaluate(const TutorSafetyPolicyRequest(
        outputText: 'Your rhythm score improved by 12.4 points since last week.',
        hasInjection: false,
        hasInventedMetric: true,
        hasCameraClaim: false,
        isRateLimited: false,
    ...
    test('camera-claim: blocks unsupported capability claim', () {
      ...
      expect(result.isAllowed, isFalse);
      expect(result.blockedCategories, contains(TutorSafetyCategory.cameraClaim));
    });
    ...
    test('unsafe-action: blocks dangerous instruction', () {
      ...
      expect(result.isAllowed, isFalse);
      expect(result.blockedCategories, contains(TutorSafetyCategory.unsafeAction));
    });
    ...
    test('usage-limit: blocks when rate limited', () {
      ...
      expect(result.isAllowed, isFalse);
      expect(result.blockedCategories, contains(TutorSafetyCategory.usageLimit));
    });
    ...
    test('redaction-required: blocks when text contains secrets', () {
      ...
      expect(result.isAllowed, isFalse);
      expect(result.blockedCategories, contains(TutorSafetyCategory.redactionRequired));
    });
  ...
  group('TutorSafetyPolicy — strictest-verdict-wins', () {
    test('returns the strictest category when multiple flags are set', () {
      ...
      expect(result.isAllowed, isFalse);
      expect(result.blockedCategories, containsAll(<TutorSafetyCategory>[
    ...
    test('injection flag dominates over other categories in priority ordering', () {
      final result = policy.evaluate(const TutorSafetyPolicyRequest(
        outputText: 'Hello.',
        hasInjection: true,
        hasInventedMetric: true,
  ...
  group('TutorSafetyPolicy — result equality', () {
    test('results with same categories compare equal', () {
      const a = TutorSafetyPolicyResult(blockedCategories: {
        TutorSafetyCategory.promptInjection,
      });
      const b = TutorSafetyPolicyResult(blockedCategories: {
        TutorSafetyCategory.promptInjection,
      });
      expect(a, equals(b));
