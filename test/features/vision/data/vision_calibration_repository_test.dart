...
void main() {
  group('round-trip', () {
    ...
    test('the top-level key order is deterministic (hand-pinned)', () async {
      final codec = const VisionCalibrationCodec();
      final encoded =
          codec.encodeToMap(profile: _profile(), guitar: _guitar());
      expect(encoded.keys.toList(), <String>[
        'schemaVersion',
        'data',
      ]);
      final data = encoded['data'] as Map<String, dynamic>;
      expect(data.keys.toList(), <String>[
        'camera',
        'guitar',
      ]);
  ...
  group('migration matrix — four cells, four separate tests', () {
    test('cell v0 — absent document returns null, not a fabricated profile',
        () async {
      final c = build();
      final read = c.repository.read();
      expect(read, isNull);
    ...
    test('cell vN — a freshly written document decodes to current schema',
        () async {
      final c = build();
    ...
    test('cell vN-1 — an older-shape document is migrated to current shape',
        () async {
      ...
      const legacyJson = '{'
          ...
          '}';
      final c = build(
        initial: {StorageKeys.visionCalibration: legacyJson},
      );
      final read = c.repository.read();
      expect(read, isNotNull);
    ...
    test('cell vN+1 — a future-version document is NOT misread, returns null',
        () async {
      ...
      final future = jsonEncode({
        'schemaVersion': 99,
        'data': {
          'camera': {'camera': 'back', 'orientation': 0, 'zoom': 0.5,
            'setupProfile': 'practiceBalanced', 'createdAt': '2026-08-07T12:00:00.000Z',
            'qualityScore': 0.9},
          'guitar': {'nut': {'x': 0.5, 'y': 0.5}, 'bridge': {'x': 0.5, 'y': 0.5},
            'neckPolygon': [], 'createdAt': '2026-08-07T12:00:00.000Z'},
        },
      });
      final c = build(
        initial: {StorageKeys.visionCalibration: future},
      );
      final read = c.repository.read();
      expect(read, isNull);
      ...
      expect(
        c.logger.events,
        contains('storage.document.corrupt'),
      );
    });
  });

  group('idempotence', () {
    test('writing the same profile+guitar twice yields identical JSON', () async {
      final c = build();
      final profile = _profile();
      final guitar = _guitar();
      await c.repository.write(profile: profile, guitar: guitar);
  ...
  group('corruption tolerance — record-level quarantine', () {
    test('csonka JSON — az egész dokumentum karanténba kerül, read null', () async {
      final c = build(
    ...
    test('hibás típus — a camera mező nem objektum → record karantén', () async {
      ...
      final c = build(
        initial: {StorageKeys.visionCalibration: malformed},
      );
      final read = c.repository.read();
    ...
    test('degenerált polygon — a neckPolygon < 3 csúcs → record karantén',
        () async {
      ...
      final c = build(
        initial: {StorageKeys.visionCalibration: degenerate},
      );
      final read = c.repository.read();
      expect(read, isNull);
      expect(c.logger.events, contains('storage.document.record_skipped'));
  ...
  group('privacy snapshot', () {
    test('a serialized profile only contains the expected key set (any level)',
        () async {
      final c = build();
      await c.repository.write(profile: _profile(), guitar: _guitar());
      ...
      expect(
        guitar.keys.toSet(),
        <String>{'nut', 'bridge', 'neckPolygon', 'createdAt'},
      );
      ...
      for (final banned in <String>[
        ...
        expect(flat.contains(banned), isFalse,
            reason: 'raw-kép-suspect mezőnév a JSON-ban: $banned');
      }
    });

    test('the privacy snapshot set is hand-pinned — adding a key fails',
        () async {
  ...
  group('real-violation probe', () {
    test('a pixel-coordinate field bypasses validation → codec rejects',
        () async {
      ...
      final c = build(
        initial: {StorageKeys.visionCalibration: malicious},
      );
      final read = c.repository.read();
