// Engine restart: stop() → start() on the SAME RealTunerEngine instance.
//
// `tunerReadingProvider` watches `tuningReferenceProvider`, so changing A4
// re-runs the provider: its `onDispose` fires `engine.stop()` WITHOUT awaiting
// it and the rebuilt body immediately calls `engine.start(a4: …)` on the very
// same engine. The stop is therefore still in flight — the microphone lease is
// not back yet and the previous continuation has not torn its ports down.
import 'dart:async';
...
void main() {
  ...
  test('an un-awaited stop() followed immediately by start() keeps the NEW '
      'session alive (A4 change restart on the same engine)', () async {
    ...
    final sub = engine.readings.listen(readings.add, onError: errors.add);
    addTearDown(sub.cancel);

    await engine.start(a4: 440);
    expect(coordinator.activeOwner, AudioOwner.tuner);

    // Exactly what the provider does on an A4 change.
    unawaited(engine.stop());
    await engine.start(a4: 442);
    ...
    // a stale stop continuation that closed the new ReceivePort or killed
    // the new isolate leaves the screen on "listening" with no reading ever.
    capture.emit(List<double>.filled(4096, 0));
    for (var i = 0; i < 100 && readings.isEmpty; i++) {
  ...
  // when the stop arrives (`ref.invalidate` then a background revoke, the
  // Retry banner then backgrounding). The stop has to stop the microphone THAT
  // start opens. An engine that fires `_mic.stop()` eagerly and only queues
  // the teardown consumes the stop SIGNAL before the start ever runs —
  // `MicCapture.start` resets `_stopRequested` — so the queued start opens the
  // microphone and the queued teardown then awaits an already-finished stop:
  // a live capture with the exclusive lease held forever (§5, E01-R09 §9.4).
  test('a start that is still queued when stop() arrives is stopped BY it '
      '(the microphone never outlives its stop)', () async {
    final coordinator = AudioSessionCoordinator();
    final capture = SlowStoppingAudioCapture();
    final engine = RealTunerEngine(
      mic: fakeMicCapture(
    ...
    // Exactly the shape a queued Resume/A4 restart followed by a background
    // revoke has: the start has not run yet when the stop is enqueued.
    unawaited(engine.start(a4: 442));
    await engine.stop();
  ...
  test('two restarts landing back to back still end with the microphone '
      'closed and the lease back (A4 changed twice)', () async {
    final coordinator = AudioSessionCoordinator();
    final capture = SlowStoppingAudioCapture();
    final engine = RealTunerEngine(
      mic: fakeMicCapture(
  ...
  // §9.3 proper: the user leaves the Tuner while the permission dialog is up.
  // The microphone must never be OPENED for a screen that is already gone —
  // on Android the runtime dialog pauses the activity, so this is the ordinary
  // first-run path, not an edge case, and "opened, then closed a step later"
  // means recording with the screen gone. That is what `stop()`'s cancel
  // signal (`MicCapture._stopRequested`, raised synchronously) is for. The
  // same path carries a background revoke
  ...
  test('stop() while the permission dialog is up never opens the microphone '
    ...
    // The user leaves the Tuner. NOT awaited: the caller (ref.onDispose) drops
    // this future, which must not turn a throwing teardown into an unhandled
    // async error either.
  ...
  // lifecycle resources or leaks them for the rest of the process (§7).
  // ---------------------------------------------------------------------

  test('a platform stop that throws still hands the exclusive lease back, '
      'logs the failure, and leaves the engine restartable', () async {
    final coordinator = AudioSessionCoordinator();
    ...
    expect(
      logger.errorEvents,
      contains('tuner.engine.lifecycle_failed'),
      reason:
    ...
    expect(
      errors,
      isEmpty,
      reason: 'a failed teardown is not a reading-stream error',
    );

    // …and the lifecycle chain is not poisoned: the next start still runs, on
    // a NEW isolate that actually delivers readings.
    capture.throwOnStop = false;
    await engine.start();
    expect(coordinator.activeOwner, AudioOwner.tuner);
    capture.emit(List<double>.filled(4096, 0));
