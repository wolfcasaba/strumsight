...
void main() {
  ...
  group('EvaluationRunner.matchEvents — known-answer cells', () {
    ...
    test('(b) every expected event is missed -> recall == 0.0, '
    ...
    test('(c) half of the expected events are missing, nothing extra '
    ...
    test('(d) 10 detected, 5 correct, 6 expected '
  ...
  group('CalibrationFitter — 30-per-bin / 300-total minimums (inclusive)', () {
    List<ConfidenceObservation> observationsWithNarrowestBin(int narrowCount) {
      ...
      for (var i = 0; i < narrowCount; i++) {
        observations.add(ConfidenceObservation(rawScore: 0.95, correct: true));
      }
      return observations;
    }
    ...
    test(
      'exactly 30 observations in the narrowest bin -> fits (inclusive)',
      () {
        final fit = const CalibrationFitter().fit(
  ...
  test('CalibrationFitter.fit computes a hand-verified Expected Calibration '
      'Error', () {
    // python3 -c "
    // n=100; w=50/n
  ...
  group('EvaluationRunner.run', () {
    test(
      'reports the nine metrics and a slice breakdown for the CI fixture',
      () {
        ...
        expect(
          report.overall.bpmErrorPercent,
          closeTo(0.8333333333333334, 1e-9),
        );
