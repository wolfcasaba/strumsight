...
void main() {
  ...
  test('an injected refiner re-labels strum directions, keeping the times', () {
    ...
    expect(
      seen,
      hasLength(result.strums.length),
  ...
  test(
    'a throwing refiner falls back to the heuristic labels (never crash)',
    () {
      final analyzer = ClipAnalyzer(
  ...
  test('runClipAnalysis wires real weights bytes end-to-end (smoke)', () {
    ...
    expect(
      refined.strums,
      hasLength(baseline.strums.length),
  ...
  test(
    'batch confidences are CALIBRATED P(correct), not raw softmax (r171)',
    () {
