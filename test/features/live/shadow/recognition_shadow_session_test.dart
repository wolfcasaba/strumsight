...
class _CountingCandidate implements ChordShadowCandidateSource {
  // `answerFrom` is the fake's "stays silent until" seam; every cell in this
  // file exercises the always-answering default, so no caller passes it.
  ...
  ChordShadowVerdict? verdictAtSeconds(double timeSec) {
    ...
    return ChordShadowVerdict(timeSec: timeSec, label: label, posterior: 0.9);
  }
}
...
void main() {
  ...
  group('the report is deterministic and bounded', () {
    test('the same PCM produces the same snapshot JSON', () {
      ...
      final first = runRecognitionShadow(
        RecognitionShadowRequest(pcm: signal, sampleRate: _sr, gate: _openGate),
      );
  ...
  group('chord agreement folds into the matrix', () {
    test('a constant candidate against silence lands in the N.C. row', () {
      ...
      expect(
        chord.countFor(ShadowChordClass.noChord, ShadowChordClass.parse('Am')),
