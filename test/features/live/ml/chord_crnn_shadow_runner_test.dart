...
void main() {
  ...
  group('activation is fail-VISIBLE', () {
    ...
    test('the shipped asset activates and reports its own hash', () {
      ...
      expect(activation.sha256, ChordCrnnShadowRunner.shippedChordModelSha256);
  ...
  group('clip decoding', () {
    ...
    test('verdictAtSeconds picks the frame covering the instant', () {
      ...
      expect(runner.verdictAtSeconds(1e6)!.timeSec, verdicts.last.timeSec);
  ...
  group('the streaming adapter is bounded', () {
    ...
    test('a filled window produces a verdict stamped on the ENGINE clock', () {
      ...
      expect(ChordCrnnShadowRunner.majmin25Labels, contains(verdict!.label));
      expect(
        verdict.timeSec,
        closeTo(seconds, 1e-3),
        reason:
            'the streaming verdict is stamped at the audio fed so far, '
