...
void main() {
  ...
  test('en and hu define exactly the same keys', () {
    final ek = keysOf(en);
    final hk = keysOf(hu);
    expect(ek.difference(hk), isEmpty, reason: 'keys missing from app_hu.arb');
    expect(hk.difference(ek), isEmpty, reason: 'keys missing from app_en.arb');
  });
