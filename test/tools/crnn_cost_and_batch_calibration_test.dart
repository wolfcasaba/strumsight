...
void main() {
  ...
  test('PROBE(b): live CRNN cost per hop and per strum', () {
    ...
    for (var i = 0; i < signal.length; i++) {
      signal[i] =
          0.4 * math.sin(2 * math.pi * 196 * i / sr) +
          0.1 * (rng.nextDouble() * 2 - 1);
    }
    final nFrames = 1 + (signal.length - window) ~/ hop;
    const features = StrumFrameFeatures(
      lowEnergy: 0,
      highEnergy: 0,
      centroid: 0,
    );
    ...
    for (var i = 0; i < runs; i++) {
      classifier.classifyAt(
        onsetFrame: nFrames - 13 - i,
        currentFrame: nFrames - 1 - i,
      );
    }
    swClassify.stop();
    ...
    // ignore: avoid_print
    print(
      'COST PROBE observe=${observeUs.toStringAsFixed(1)}us/hop '
    ...
    expect(
      ...
      lessThan(60),
      reason:
          'JIT classify regressed far past the measured 33 ms — '
  ...
  test(
    ...
    () {
      ...
      // ignore: avoid_print
      print(
        'BATCH CALIB ${[for (var b = 0; b < n.length; b++) '${b == 0
                  ? '<${edges[0]}'
                  : b == n.length - 1
                  ? '>=${edges.last}'
                  : '${edges[b - 1]}-${edges[b]}'}'
              ' -> n=${n[b]} acc=${(100 * ok[b] / (n[b] == 0 ? 1 : n[b])).round()}%'].join(', ')}',
      );
